                ...
                session.executeSafe(s"select * from wallets.wallet_by_user_id where userId = '$userId' and programId = '$programId';").toList.headOption match {
                  case Some(walletRow) =>
                    ...
                    val currentEarnProfiles: Set[EarnProfile] =
                      programs.find(_.startDateTime == "base").getOrElse(throw new Exception("soeee")).levels.find(_.level == currentLevel).getOrElse(throw new Exception("asddd")).earnProfiles.filter(_.userType == userType)
                    def calculatePoints(earnProfiles:Set[EarnProfile], tanks: Set[TankSummary]): String ={
                      earnProfiles.flatMap{profile: EarnProfile =>
                        tanks.filter(_.name == profile.tank)
                          .map(tank => TankSummary(tank.points + (req.amountInBase * profile.earnRate).toInt, tank.name))
                      }.asJson.noSpaces
                    }
