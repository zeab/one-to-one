...
object CommandAlias {
  ...
  val allDockerAlias: Seq[Def.Setting[_]] =
    dockerCommands("u", usersKey) ++
      dockerCommands(usersKey) ++
      dockerCommands("w", walletsKey) ++
      dockerCommands(walletsKey) ++
      dockerCommands("t", transactionsKey) ++
  ...
  val allAssemblyAlias: Seq[Def.Setting[_]] =
    assemblyCommands("u", usersKey) ++
      assemblyCommands(usersKey)
